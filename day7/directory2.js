...
let currentAvailable = totalDisk - allDirPaths['/'];
console.log(`current available space ${currentAvailable}`);

let requiredToDelete = spaceRequired - currentAvailable;
console.log(`space required ${requiredToDelete}`);

let currentSmallestDirSizeToDelete = allDirPaths['/'];

for (var key in allDirPaths) {
  var dirSize = allDirPaths[key];

  if (dirSize >= requiredToDelete) {
    if (dirSize < currentSmallestDirSizeToDelete) {
      currentSmallestDirSizeToDelete = dirSize;
    }
  }
}

console.log(`smallest single dir to delete ${currentSmallestDirSizeToDelete}`);
